...
app.post('/create-checkout-session', async (req, res) => {
  const { bookingId, carModel, pricePerDay, days } = req.body;
  ...
  try {
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: `Car Rental: ${carModel}`,
            },
            unit_amount: pricePerDay * 100, // Stripe expects amount in cents
          },
          quantity: days,
        },
      ],
    ...
    console.error('Error creating checkout session:', error.stack);
